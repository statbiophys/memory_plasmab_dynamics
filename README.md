...
Within the two folders **Phad data** and **Mikelov data** there are numbered scripts that can 
 - **Script number 1**: download the fastq files
 - **Script number 2 to 5**: process the fastq files, align the IG sequences.
 - **Script number 6**: cluster the sequences into clonal families.
 - **Script number 7**: run the noise inference and plot the marginals
 - **Script number 8**: run the Geometric Brownian Motion inference for memory cells and plot the marginals
 - **Script number 9**: run the inference of the coupled memory-plasmablast system and plot the marginals
...
We can split the analysis in two parts:
...
The last command execute the jupyter, but you still need to link that to your favourite browser. You just need to copy one of the two urls printed at terminal below the sentence `Or copy and paste one of these URLs:`, and copy it on your browser.
