...
integrator_negbin = lf.integr_plaw_negbin

def _aux_log_like_paral_negbin(args):
    ns, n_count, alpha, fmin, a, b, N_reads = args
    pars = lf.plaw_negbin_pars(alpha, fmin, a, b, N_reads)
    like_n = integrator_negbin(pars, ns)
    return np.log(max(like_n, 1e-300)) * n_count


def log_like_negbin_parall(ns_uniq, ns_count, pars, n_threads=8):
    like_0 = integrator_negbin(pars, np.zeros(len(ns_uniq[0]), dtype=int))
    N_obs = sum(ns_count)
    N = N_obs / (1 - like_0)
    args = [(ns, n_count, pars.alpha, pars.fmin, pars.a, pars.b, pars.Ms) for ns, n_count in zip(ns_uniq, ns_count)]
    lls = parallelize_async(_aux_log_like_paral_negbin, args, n_threads)
    return np.sum(lls) - N_obs * np.log(max(1 - like_0, 1e-30)), N
...
def hess_plawpoiss_M(ns_uniq, ns_count, alpha, fmin, N_cells):
    """
    ...
    pars = lf.plaw_poiss_pars(alpha, fmin, N_cells)
    for ns, n_count in zip(ns_uniq, ns_count):
...
def hess_plawpoiss(ns_uniq, ns_count, alpha, fmin, N_cells):
    """
    Hessian of the log likelihood of the power-law poisson integral with transformed variables:
...
def jac_plawpoiss(ns_uniq, ns_count, alpha, fmin, N_cells):
    """
...
def hess_plawnegbin_paral(ns_uniq, ns_count, pars, n_threads=8):
    args = [(ns, n_count, pars.alpha, pars.fmin, pars.a, pars.b, pars.Ms) for ns, n_count in zip(ns_uniq, ns_count)]
    hess = np.sum(parallelize_async(_aux_hess_paral_negbin, args, n_threads), axis=0)
    hess -= np.array(lf.hess_log_integr_plaw_negbin(pars, np.zeros(len(pars.Ms), dtype=int))) * np.sum(ns_count)
    return hess
...
def jac_plawnegbin(ns_uniq, ns_count, alpha, fmin, a, b, N_cells):
    """
    Jacobian of the log likelihood of the power-law poisson integral with transformed variables:
    log10 fmin.
    """
    g = np.zeros(5)
    pars = lf.plaw_negbin_pars(alpha, fmin, a, b, N_cells)
    for ns, n_count in zip(ns_uniq, ns_count):
